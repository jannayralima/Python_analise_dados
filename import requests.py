...
with sqlite3.connect(f'{pasta}{bancoDados}') as conn:
    ...
    for filme in filmes:
        try:
            cursor.execute('''
            INSERT OR IGNORE INTO filmes (Titulo, Direcao, Nota, Link e Categoria) VALORES (?, ?, ?, ?, ?, ?)
            ''', (
                filme['titulo'],
                filme['Direcao'],
            ...
            print(f"Erro ao inserir filme {filme['titulo']} ou banco de dados\n Erro: {e}")   
    conn.commit()
termino = datetime.datetime.now()

print("-------------------")
print(" Dados Raspados com sucesso")
print(f"\nArquivo CSV salvo em {pasta}{bancoDados}")
print(f"\n Dados Armazenados no banco de dados {bancoDados}")
print(f" Tarefa Iniciada em: {agora.strftime('%H:%M:%S')}")
print(f" Tarefa Finalizada em: {agora.strftime('%H:%M:%S')}")
print("Obrigada por usar o BotFilmes")
print("---------------------------")